...
| MP-03 | Muhasebe çekirdeği ve cari ilk dikey dilim | validating | Kaynak olaydan rapora uzlaşan uçtan uca akış | Golden senaryo UAT kabulü |
| MP-04 | Stok ve satış çekirdeği | in-progress | Sipariş, rezervasyon, sevk, fatura, iade | Miktar ve değer mutabakatı |
...
Giriş kapısı güncellemesi (31 Ağustos 2026): Repository sahibi `DEC-MP01-001`–`010`, `012` ve `021`–`024` kapsamında tenant/company, organizasyon boyutları, takvim yılı, TRY fonksiyonel para, günlük manuel kur, hassasiyet, 120/320 geliştirme şablonu, kesin kayıt düzeltmesi, cari allocation/aging, banka transit/reconciliation, granüler permission, opening open-item, manuel Party refresh ve MP-sonu test kadansını yazılı olarak belirledi. KKTC 27/1977 Vergi Usul Yasası madde 114, normal hesap döneminin takvim yılı olduğunu resmi kaynaktan doğrular. Bu kanıtlarla cari, muhasebe, banka mutabakatı ve rapor refresh karar-backed dilimleri `in-progress` ilerleyebilir. İsimli mali/güvenlik kabulü, resmi hesap eşlemeleri, vergi/e-Fatura, production no-go maddeleri ve MP kapanış birleşik testi kendi kapsamlarını bloklamaya devam eder; faz tamamlanmış sayılmaz. Yaşayan geçiş planı [MP-01 firma politikası temel setindedir](docs/project/plans/2026-08-27-mp01-business-policy-baseline.md).

Test kadansı kararı (31 Ağustos 2026): `DEC-MP01-024` ile her oturumda full repository regresyonu çalıştırma uygulaması kaldırıldı. Her davranış diliminde değişen finansal/güvenlik invariantı için dar hedef testi ve gerekli compile/static kontrol korunur. Solution-wide unit/integration, gerçek PostgreSQL/RLS/concurrency, boş+mevcut migration, restore, golden cross-foot, web, Android ve güvenlik paketi ilgili MP `validating`/kapanış kapısında tek birleşik koşu olarak çalışır; bulgular aynı MP içinde düzeltilip tam paket yeniden çalıştırılır.
...
Reconciliation approval ve TRY transit→banka GL composition kanıtı (31 Ağustos 2026): `0041` exact proposal/version, gerçek maker'dan farklı tek yönetici, `0,00` tolerans, immutable payment identity ve global statement/payment participant lock'larını append-only approved sonuca bağladı. Treasury approved fact loader, statement booking date ve payment drill-down kimliklerini modüller arası contract ile Accounting'e taşır. Accounting, hard-code hesap kullanmadan incoming/outgoing transit kapanışlarını authoritative kur/yuvarlama snapshot'lı dengeli statement journal'larına dönüştürür; Bootstrap approval persistence ile bütün prepare/post sonuçlarını caller'ın tek PostgreSQL transaction'ında birleştirir. İlk dilim yalnız TRY identity-rate'tir. Yabancı para FX difference source event'i, gerçek PostgreSQL rollback/idempotency/RLS golden'ı ve Windows Application Control nedeniyle çalışamayan dar runtime kontrolü MP validating kapısında açık olduğundan backlog 17 ve MP-03 `in-progress` kalır.

Multi-currency payment temeli (1 Eylül 2026): Önceki identity-rate payment sınırı `DEC-MP01-004`–`006` doğrultusunda genişletildi. Payment source artık işlem/TRY functional para, günlük company-effective rate kimlik/sürümü, AwayFromZero rounding policy kimlik/sürümü, 12 haneli unrounded functional tutar ve explicit residual taşır; domain ve authoritative loader stored sonucu snapshot'tan exact yeniden üretir. `0042` uygulanmış `0022`'yi değiştirmeden eski kayıtları backfill eder ve deterministic conversion DB guard'ını ekler. Etkilenen bütün .NET hedefleri `0 warning / 0 error` derlendi; gerçek PostgreSQL migration/tamper paketi MP validating'de açıktır. Approved reconciliation contract'ına payment-date functional kanıtın taşınması ve statement-date realized FX journal'ı tamamlanmadığından backlog 17/MP-03 `in-progress` kalır.

Reconciliation realized FX composition kanıtı (1 Eylül 2026): Approved Treasury fact matched payment'ın payment-date transaction/functional kapasitesi ile rate/rounding kimlik sürümünü taşır; Accounting banka satırını statement-date kurundan, transit satırlarını payment-date kurundan üretir ve farkı mapped realized gain/loss hesabında functional identity snapshot'lı ayrı satırla dengeler. Split payment functional parçaları original kapasiteyi exact kapatmıyorsa fail-closed davranır. Contracts, Treasury Infrastructure, Accounting Application, Bootstrap, Unit, Integration ve Architecture derlemeleri `0 warning / 0 error` geçti. Gerçek PostgreSQL `0042` ve approval→FX journal rollback/idempotency/RLS golden'ı MP validating paketinde açık olduğundan backlog 17 ve MP-03 `in-progress` kalır.
...
Exact Party→GL projection composition kanıtı (29 Ağustos 2026): Party source checksum V2 rapora dahil her aktif Accounting journal'ının ID ve exact source type/event/version/purpose + effective/recorded/posted lineage'ını kapsıyor. Reporting control-evidence portu artık exact source batch'i zorunlu alıyor; Accounting-owned loader bu journal setinin seçili control-account satırlarını ve reversal kesimini authoritative PostgreSQL'de yeniden doğruluyor. Rapor parası fonksiyonel paradan farklıysa line'ın immutable transaction-currency snapshot'ı kullanılıyor; yanlış para kanıtı fail-closed. Job aynı scope/slice içinde bile sıfır olmayan subledger−GL farkını publication öncesi typed fail-closed durduruyor. Fonksiyonel parası TRY olan gerçek `75 GBP` due source source→statement/aging→GL transaction görünümü→atomic sink zincirinden geçti, persisted iki cross-foot tekrar yüklendi ve command replay'i yeni fact üretmedi. Aynı fixture'da `25 opening + 75 due + 10 unallocation - 10 allocation = 100 GBP` cari/GL sıfır fark verdi; wrong-company, wrong-currency ve eksik control-line negatifleri geçti. Opening event due-date/payment-term/open-item kimliği taşımadığından opening dahil aging/allocation semantiği tahmin edilmedi; full golden ve MP-03 çıkış kapısı bu ürün kararı ile diğer açık banka/API/restore kanıtları nedeniyle `in-progress` kalır.

Authoritative aging-policy seçimi kanıtı (29 Ağustos 2026): Şirketin tek policy kimliğinde ardışık, append-only sürümlerini effective date ve recorded cutoff kesiminde seçen [source planı](docs/project/plans/2026-08-29-authoritative-aging-policy-source.md) tamamlandı. `0036+0037` migration'ları tam integer-day bucket coverage, policy kimliği, sürüm sırası ve recorded chronology guard'larını; forced tenant/company RLS ile runtime SELECT/INSERT fakat UPDATE/DELETE yokluğunu kurdu. Stream serialization, security-invoker transaction advisory lock kullanarak caller RLS'ini aşmıyor. Gelecekte kaydedilmiş/sadece gelecekte etkili sürüm mevcut kesime sızmadı, runtime append-only insert geçti; cross-company okuma ve bozuk stream/coverage reddedildi. Gerçek `75 GBP` Party→aging→GL golden job artık fixed fixture yerine bu PostgreSQL kaynağını kullanıyor. Mevcut DB son yükseltmede `1/0`, boş DB `37/0` migration ve tam RLS integration paketi geçti. Opening-aging kararı, production permission/API, Worker schedule ve diğer MP-03 kapıları açık olduğundan faz `in-progress` kalır.

Production Party report API query kanıtı (30 Ağustos 2026): `party.account.detail` version `1` tanımı immutable `reporting.party-account.view` permission'ına bağlandı. `POST /api/v1/reports/{code}/queries`, trusted company scope ve permission'ı persisted statement/aging lookup'tan önce denetliyor; allowed, denied ve not-found sonuçlarını projection okumasıyla aynı PostgreSQL transaction'ında append-only audit'e yazıyor. Denial target kimliği taşımıyor, audit başarısızsa mali yanıt dönmüyor. API yalnız Reporting Application portuna bağlı; PostgreSQL adapter ve ortak transaction-bound audit appender Bootstrap composition root'ta bağlandı. Explicit response aynı generation'ın statement satırlarını, source lineage'ını, aging policy/bucket/item ve exact closing/remaining cross-foot'unu taşıyor; projection refresh veya source mutation yapmıyor. Gerçek PostgreSQL tenant/company RLS paketi, `63` domain check, `20` source-project architecture/API check, Release build `0 warning/error`, format ve migration tekrarı `0/0` geçti. Production permission/API açığı kapandı; opening-aging ürünü kararı, Worker schedule, banka ve restore/golden çıkış kanıtları nedeniyle MP-03 `in-progress` kalır.

Party report durable Worker doğrulama kanıtı (31 Ağustos 2026): `DEC-MP01-020`, background consumer'ı insan kullanıcıdan ayrık service identity, tek tenant, deployment allow-list'indeki her şirket için aktif IAM izni ve `reporting.party-account.refresh` permission'ıyla sınırlar; IAM'deki deployment dışı izin bu örneğin etkin kapsamına girmez. `0038+0039`, forced-RLS IAM kayıtlarını ve `SKIP LOCKED` claim, lease/reclaim, bounded retry, terminal failure ile append-only transition eventli durable occurrence kuyruğunu ekledi. Bootstrap, gerçek Party source→authoritative aging policy→exact GL evidence→atomic sink zincirini Worker'a bağladı ve scope'u publish öncesi yeniden doğruluyor. Queue concurrency/restart/revocation/overbroad/cross-company negatifleri gerçek PostgreSQL'de geçti. İlk production golden, eski tenant-geneli statement-event unique kısıtının aynı ekonomik olayı ikinci immutable generation'da engellediğini yakaladı; uygulanmış migration değiştirilmeden `0040` ile tekillik statement kapsamına alındı. `0040` mevcut test DB'sinde checksum ve catalog üzerinden doğrulandı, Release build `0 warning/error` geçti. Son Worker golden, boş DB ve 0/0 runtime tekrarları Windows Application Control `0x800711C7` engeli nedeniyle validating durumundadır; politika bypass edilmedi. Recurring schedule authoring/API, production service identity provisioning, opening-aging ürünü kararı, banka ve restore çıkış kanıtları açık olduğundan MP-03 `in-progress` kalır.

Opening open-item uygulama kanıtı (31 Ağustos 2026): `DEC-MP01-021` ile yeni PartyAccount opening kaynakları explicit bir/çok due date ve payment-term snapshot satırı, tekil satır kimliği ve source amount'a exact eşit toplam taşımak zorundadır. Writer authoritative PartyAccount doğal yönünü zorunlu kılar ve opening event + mevcut `due_schedule` modelini aynı transaction'da idempotent saklar; yeni migration gerekmez ve legacy opening'e vade tahmin edilmez. Report source due-schedule bağlı opening'i eski toplama dahil etmeyip normal allocatable/aging open-item olarak tek kez üretir. Unit/Integration projeleri `0 warning/error` derlendi ve diff kontrolü temizdir; dar runtime ve gerçek PostgreSQL opening→aging→GL tekrarları Windows Application Control `0x800711C7` nedeniyle MP validating kapısında açık kanıttır. MP-03 durumu değişmez.

Birleşik MP-03 kapanış kanıtı (2 Eylül 2026): `scripts/verify.ps1` bulguları aynı faz içinde düzeltilip paket baştan sona yeniden çalıştırıldı. NuGet proje kilitleri güncellendi; golden payment fixture'ı effective-date kur kanıtına bağlandı; worker fixture'ında parolasını gizleyen `NpgsqlDataSource.ConnectionString` yerine özgün app connection string kullanıldı; `0042_payment_currency_conversion_snapshot` migrator kataloğuna eklendi; Bootstrap'ın Treasury Domain'e doğrudan proje referansı kaldırıldı ve restore betiği Compose database değişkenini kendi `.env` kaynağından bağımsız kurar hale getirildi. Son birleşik koşuda Release solution build `0 warning/error`, `67` domain kontrolü, `22` source-project architecture/API/authorization/telemetry kontrolü, format, web lint/typecheck/`6` Vitest/production build, mevcut PostgreSQL `0/0` migration + tenant/company RLS integration, boş PostgreSQL `42/0` migration + RLS/integration, Keycloak token→ERP scope/permission smoke, izole PostgreSQL restore + migration/scope/outbox/auth smoke ve Android lint/unit/instrumentation APK build kapılarının tamamı geçti. Docker Desktop'ı bloklayan 26 Ağustos tarihli erişilemeyen geçici `run` klasörü silinmeden `run-stale-20260902-1530` adıyla yedeklendi ve engine sağlıklı başlatıldı. Teknik otomasyon kapısı tamamlandı; süreç sahibiyle yapılacak golden UAT kontrolü ve kabul kaydı bulunmadığından MP-03 `completed` değil `validating` durumundadır.
...
7 Eylül 2026 karar kapısı: DEC-MP01-025 ile kullanıcı seçili depo, mevcut miktar kadar kısmi
rezervasyon ve otomatik expiry olmaması kabul edildi. Miktar rezervasyonu persistence'ının
ürün kararı blokajı kalktı; uygulama ve concurrency kanıtı bekleniyor. DEC-MP01-011 değerleme,
backdate ve sayım kararları açık kalır. Faz `in-progress` durumundadır.

Amaç: Miktar, maliyet, cari ve muhasebe etkisini tek satış zincirinde uzlaştırmak.

Giriş kapısı güncellemesi (2 Eylül 2026): MP-03 birleşik teknik otomasyon kapısı geçti ve faz golden UAT kabulini bekleyen `validating` durumuna geldi. `DEC-MP01-011` stok değerleme, eksi stok, backdate/repost ve sayım politikalarını açık bırakırken generic quantity invariant ve impact-preview contract dilimlerine izin verir. Bu sınırla Inventory Domain başlatıldı: PostgreSQL `numeric(20,6)` uyumlu decimal miktar, tenant/company + exact source event/line/version/purpose bağlı signed stock movement ve farklı depolar arasında source issue + destination receipt toplamını exact sıfır yapan immediate transfer modeli eklendi. Geri tarihli hareketler için effective-date + deterministic-sequence pozisyonu, valuation watermark generation/cutoff/checksum kanıtı, etkilenen aralık ve operational/inventory valuation/GL/tax/hard-legal kilit kapsamlarının tamamını isteyen immutable impact preview eklendi; bu contract işleme izni, maliyet sonucu veya repost üretmez. Ayrıca typed/canonical base UOM, tenant seviyesinde ürün tanımı ve şirket bazlı activation/version sözleşmesi eklendi; stock/non-stock/service/expense sınıfları, lot/seri uygunluğu ve 0-6 basamak miktar ölçeği fail-closed doğrulanır. `0043` migration'ı company-scoped warehouse, item activation ve append-only stock movement persistence temelini forced RLS, immutable master identity/version, exact item-UOM FK, source/position uniqueness ve deferred immediate-transfer balance guard ile kurar; gerçek PostgreSQL senaryosu yazılmış fakat MP-04 toplu kapısına kadar çalıştırılmamıştır. Maliyet yöntemi/profili, negatif stok, iki aşamalı transit ve gerçek backdate/repost davranışı seçilmedi. Inventory Domain, Migrator ve Integration harness Release derlemeleri `0 warning/error`, architecture/API kontrolü `23` source project için geçti. Yeni `INV-` runtime kontrolleri Windows Application Control `0x800711C7` nedeniyle başlayamadı; politika bypass edilmedi ve tüm runtime/migration kanıtı MP-04 validating paketine açık borç olarak taşındı. Yaşayan plan [Inventory quantity/movement foundation](docs/project/plans/2026-09-02-inventory-quantity-movement-foundation.md) belgesidir.

Teknik kanıt güncellemesi (4 Eylül 2026): Inventory Infrastructure proje sınırı ile execution-scope zorunlu, caller transaction'ına katılan immediate-transfer writer eklendi. Writer savepoint sayesinde source unique conflict durumunda olası tek taraflı insert'i geri alır; iki canonical hareketin tüm immutable içeriği aynıysa ilk transfer kimliğini idempotent döndürür, farklıysa typed conflict üretir. Gerçek PostgreSQL senaryosu create/replay/conflict davranışını kapsayacak biçimde hazırlandı. Inventory Infrastructure, Integration ve Architecture Release derlemeleri `0 warning/error`; runtime ve migration uygulama kanıtı kararlaştırılan MP-04 toplu test kapısına bırakıldı.

Yetki kanıtı güncellemesi (4 Eylül 2026): Inventory Application sınırında `inventory.transfer.post` permission'ı, company scope ve hem kaynak hem hedef warehouse scope'u fail-closed doğrulayan authorization candidate eklendi; persistence writer yalnız bu evidence'ı kabul eder. Permission/company/warehouse negatifleri `INV-AUTH-001` altında hazırlandı. Unit, Integration ve Architecture Release derlemeleri `0 warning/error`; runtime sonucu MP-04 toplu kapısına bırakıldı.

Authoritative depo kapsamı güncellemesi (4 Eylül 2026): Command tarafından taşınan ham warehouse ID listesi güvenilir kaynak olmaktan çıkarıldı. `iam.user_warehouse_scope` tarih etkili ataması tenant/company/user/warehouse FK'leri ve actor-bound forced RLS ile `0043` migration'ına eklendi. Transaction-local trusted context kuran PostgreSQL loader immutable tenant/company/actor evidence'ı üretir; candidate bu evidence birebir eşleşmeden ve iki depo da atanmış olmadan kurulamaz. Atamasız aktör RLS negatifi hazırlandı. Inventory Application, Infrastructure, Unit ve Integration Release derlemeleri `0 warning/error`; runtime/migration kanıtı MP-04 toplu kapısına bırakıldı.

On-hand miktar sorgusu güncellemesi (4 Eylül 2026): `inventory.quantity.view`, company ve authoritative warehouse kapsamlı sorgu; append-only hareketleri explicit effective-as-of ile UTC recorded-cutoff kesişiminde item/warehouse/base-UOM bazında exact `numeric(20,6)` toplama dönüştürür. Sorgu maliyet, değerleme, available veya negatif stok kararı üretmez. Yetki atamaları read/write transaction'ında yeniden yüklenip `FOR SHARE` kilitlenir; stale depo evidence'ı reddedilir. Cutoff öncesi görünmezlik ve immediate transferin depo bazlı `-10/+10` sonucu PostgreSQL senaryosuna eklendi. Infrastructure, Unit ve Integration Release derlemeleri `0 warning/error`; runtime/migration kanıtı MP-04 toplu kapısına bırakıldı.

Hareket zaman çizelgesi güncellemesi (4 Eylül 2026): `inventory.movement.view` permission'ı altında exact item, effective-date aralığı, UTC recorded cutoff ve 1–200 sınırında deterministik cursor pagination taşıyan scoped hareket sorgusu eklendi. Sonuç source event/line/version/purpose, signed quantity, transfer ve karşı-depo lineage'ını korur; maliyet veya GL verisi taşımaz. PostgreSQL senaryosu iki transfer hareketini tek-satırlık iki sayfada yinelenmeden, exact sıfır toplam ve aynı source lineage ile yeniden kurar. Infrastructure, Unit ve Integration Release derlemeleri `0 warning/error`; runtime kanıtı MP-04 toplu kapısına bırakıldı.

Transfer reversal güncellemesi (4 Eylül 2026): Posted immediate transfer satırlarını değiştirmek yerine ters yönde yeni transfer ve iki original movement'e birebir reversal bağlantısı eklendi. Domain çiftin iki reversal linkini birlikte taşımasını ister; `0043` self-FK, original başına tek reversal ve deferred exact item/warehouse/UOM/karşı miktar ile recorded-time sırası guard'larını uygular. Yanlış karşı miktarın commit reddi ve doğru reversal sonrası iki depo on-hand toplamının sıfıra dönmesi PostgreSQL senaryosuna eklendi. Domain, Infrastructure, Unit ve Integration Release derlemeleri `0 warning/error`; runtime/migration kanıtı MP-04 toplu kapısına bırakıldı.

Satış siparişi lifecycle başlangıcı (4 Eylül 2026): Sales Domain proje sınırı açıldı. `SALES-ORD-001` altında draft→submit→approve→confirm→partial/full fulfilment→close ana yolu ile reject/revise/withdraw/cancel dalları exact expected-version üzerinden her seferinde tek sürüm artıran append-only transition event üretir. Event actor, UTC occurrence, correlation, previous/new status ve version ile gerekçeyi korur; submitted version maker'ı kendi kaydını approve/reject edemez. Bu dilim siparişi yalnız commitment sayar; stok, rezervasyon, fiyat, vergi, cari veya GL sonucu üretmez. Sales Domain, Unit ve Architecture Release derlemeleri `0 warning/error`; runtime sonucu MP-04 toplu kapısına bırakıldı. Yaşayan plan [Sales order lifecycle foundation](docs/project/plans/2026-09-04-sales-order-lifecycle-foundation.md) belgesidir.

Sipariş fulfilment evidence güncellemesi (4 Eylül 2026): `SALES-FUL-001` ile partial/full lifecycle geçişleri caller beyanı yerine aynı tenant/company/order içindeki unique order-line→dispatch-line allocation toplamına bağlandı. Pozitif `numeric(20,6)` quantity ve canonical base-UOM korunur; allocation başka satıra bağlanamaz veya ordered quantity'yi aşamaz. Eksik evidence ve partial miktarla full geçiş fail-closed kalır. Bu domain evidence henüz authoritative persistence veya stok hareketi değildir. Sales Domain ve Unit Release derlemeleri `0 warning/error`; runtime sonucu MP-04 toplu kapısına bırakıldı.

Sipariş kalıcılık güncellemesi (4 Eylül 2026): `SALES-ORD-003` ile tenant/company forced-RLS current projection ve append-only transition history `0044` migration'ına eklendi. Exact `+1` optimistic version, legal status/transition eşleşmesi, submit maker devri, approve/reject maker-checker, reject/revise/cancel gerekçesi ve event↔projection atomikliği deferred DB guard'larıyla korunur. Permission/company scope doğrulayan Application command'ları ve caller transaction'ına katılan PostgreSQL writer exact correlation retry'ı ilk immutable sonuç olarak döndürür; farklı içerik ve stale version fail-closed kalır. Persisted allocation olmadığı için fulfilment yazımı Application sınırında kapalıdır. Cross-company RLS, create/transition replay-conflict ve DELETE privilege negatifleri gerçek PostgreSQL senaryosuna hazırlandı. Sales Application, Infrastructure, Unit, Integration, Architecture ve Migrator Release derlemeleri `0 warning/error`; runtime/migration kanıtı kararlaştırılan MP-04 toplu kapısına bırakıldı.

Sipariş lifecycle okuma güncellemesi (4 Eylül 2026): `SALES-ORD-004` ile `sales.order.view` permission/company scope isteyen detail query ve PostgreSQL loader eklendi. Loader current header'ı `FOR SHARE` kilitledikten sonra aynı caller transaction'ında version sıralı append-only geçmişi okur; Application view contract'ı eksik, sıra dışı veya current state ile uyuşmayan event zincirini fail-closed reddeder. Submit→approve→confirm timeline rekonstrüksiyonu ve başka-company not-found negatifi MP-04 gerçek DB senaryosuna eklendi. İlk Unit analizör koşusundaki `CA1806` fixture hatası düzeltildi; Sales Infrastructure, Unit, Integration ve Architecture Release derlemeleri `0 warning/error`. Runtime/migration kanıtı MP-04 toplu kapısına bırakıldı.

Sipariş API güncellemesi (4 Eylül 2026): `SALES-API-001` ile `/api/v1/sales-orders` create/detail ve fulfilment hariç açık lifecycle action uçları eklendi. Create ve transition UUID `Idempotency-Key`, transition ayrıca quoted positive `If-Match` ister; ETag/Location ve güvenli 404/409/412/422/503 Problem Details eşlemeleri tanımlıdır. API PostgreSQL görmez, Application gateway üzerinden çalışır; Bootstrap gerçek/unavailable implementasyonu seçer ve allowed authorization audit lifecycle değişikliğiyle aynı transaction'da yazılır. API contract harness route/header/action/DTO sınırlarını doğrular. Restore sandbox ağ kısıtı sonrası izinli tekrarında geçti; API, Integration, Unit ve Architecture Release derlemeleri `0 warning/error`. OpenAPI üretimi ile HTTP/runtime kanıtı MP-04 toplu kapısına açık borçtur.

Sipariş OpenAPI güncellemesi (4 Eylül 2026): Birinci taraf .NET 10 OpenAPI paketleri merkezi/locked eklendi ve API build'i `docs/openapi/KaguERP.Api.json` altında OpenAPI 3.1.1 üretir. Sales operationId, bearer auth, request/response şemaları, transition action enum'u, zorunlu idempotency/concurrency header'ları ve Problem Details cevap matrisi architecture drift kapısına bağlandı. Build-time process dış OIDC ayarını okumaz; normal API fail-fast auth doğrulaması değişmedi ve runtime spec yalnız Development'ta yayımlanır. API ile Architecture Release derlemeleri `0 warning/error`; generated TS/Kotlin istemciler ve HTTP/runtime kanıtı MP-04 toplu kapısına açık borçtur.

Generated istemci güncellemesi (4 Eylül 2026): OpenAPI Generator `7.24.0` ve npm launcher `2.40.1` sabitlenerek aynı OpenAPI 3.1 kaynağından `packages/api-client-ts` ile `apps/android/generated/api-client` çıktıları üretildi. Güvenli script generator/JAR preflight sonrasında yalnız iki exact generated dizini temizler. Sales create/detail/transition çağrıları typed UUID idempotency, quoted concurrency version, action allowlist, response modelleri ve bearer gereksinimini taşır; architecture drift kontrolü generator sürümü ve kritik imzaları denetler. Web adaptörü same-origin cookie/BFF kullanır ve browser tokenı kabul etmez; Android adaptörü explicit HTTPS base URL (yalnız emulator loopback HTTP istisnası), injected `Call.Factory` ve güvenli auth katmanından token provider ister. TS client build, web strict typecheck, Android `compileDebugKotlin` ve Architecture Release build'i geçti; gerçek API/client smoke ve tüm runtime regresyon MP-04 birleşik kapanış paketine açık borçtur. Upstream OpenAPI 3.1 generator desteği beta olduğundan sürüm yükseltmesi kontrollü drift incelemesi ister.

Authoritative sipariş satırı güncellemesi (5 Eylül 2026): `SALES-ORD-002A` ile satış taslağı artık 1–500 unique order-line, şirket için aktif item, canonical base-UOM ve pozitif `numeric(20,6)` miktardan oluşan immutable commitment ile atomik yaratılır. `0045` migration'ı header+line bütünlüğünü deferred guard, forced RLS, item-company/UOM FK'leri ve runtime rolünde UPDATE/DELETE yasağıyla korur. Create idempotency replay satırların immutable içeriğini de exact karşılaştırır; farklı tekrar conflict üretir. Lifecycle detail/transition persisted satırları döndürür; miktar OpenAPI ve generated TS/Kotlin DTO'larında decimal string olarak kalır. İlgili .NET Release derlemeleri `0 warning/error`, client generation geçti; runtime/migration ve sözleşme çalıştırmaları kararlaştırılan MP-04 toplu kapanış kapısına bırakıldı. Reservation orchestration halen sonraki blokajsız dilimdir.

Rezervasyon lifecycle başlangıcı (5 Eylül 2026): `INV-RES-001` ile Inventory-owned reservation state; versioned demand type/id/line/version, exact tenant/company/item/warehouse/base-UOM ve pozitif `numeric(20,6)` miktara bağlandı. `active → partially_consumed → consumed | released | expired` geçişleri exact version, actor, correlation, PostgreSQL-safe UTC occurrence ve append-only event üretir; kalan miktarı aşan consume, terminal mutation, gerekçesiz release ve erken expiry fail-closed kalır. Bu dilim persistence/available sonucu veya Sales confirm orchestration üretmez; depo seçim zamanı, expiry ve stock-position lock politikaları seçilmeden stok ayrıldığı iddia edilmez. Inventory Domain ve Unit Release derlemeleri `0 warning/error`; runtime MP-04 toplu kapısına bırakıldı. Yaşayan plan [Inventory reservation lifecycle foundation](docs/project/plans/2026-09-05-inventory-reservation-lifecycle-foundation.md) belgesidir.

Rezervasyon yetki sınırı güncellemesi (5 Eylül 2026): `INV-RES-002` ile reservation create adayı `inventory.reservation.create`, exact company scope ve transaction içinde yeniden yüklenecek actor-bound authoritative warehouse evidence olmadan üretilemez. Ham request depo listesi yetki kanıtı değildir; atanmamış depo ve farklı actor evidence'ı fail-closed reddedilir. Candidate demand source varlığı veya available sonucu iddia etmez; bunlar published module contract ve position-lock persistence dilimlerine bırakıldı. Inventory Application ve Unit Release derlemeleri `0 warning/error`; runtime MP-04 toplu kapısına bırakıldı.

Rezervasyon demand evidence güncellemesi (5 Eylül 2026): Reservation adayı exact source type/id/line/version, item, base-UOM ve azami reservable miktar taşıyan producer evidence'ına bağlandı. Stale source version, farklı item/UOM ve demand kapasitesini aşan miktar fail-closed reddedilir. Evidence'ın production otoritesi olması için persistence orchestration onu caller request'inden değil üretici modülün published contract adaptöründen aynı transaction bağlamında yükleyecektir; bu adapter henüz eklenmedi ve candidate available sonucu üretmez. Inventory Application ve Unit Release derlemeleri `0 warning/error`.

Sales reservation source güncellemesi (5 Eylül 2026): Modüller arası doğrudan tablo okumasını önlemek için bağımsız `Sales.Contracts` katmanı açıldı. Sales-owned transaction-bound PostgreSQL source yalnız `sales.order.confirm` permission/company scope altında ve `confirmed` exact order version için immutable order-line/item/base-UOM/decimal quantity snapshot'ı yayımlar; approved fakat unconfirmed, stale version ve yetkisiz actor fail-closed kalır. Inventory henüz bu contract'ı persistence orchestration'a bağlamaz ve Sales tablosu okumaz. Contracts, Sales Infrastructure, Integration ve Architecture Release derlemeleri `0 warning/error`; runtime MP-04 toplu kapısına bırakıldı.

Sales → Inventory reservation evidence güncellemesi (5 Eylül 2026): `INV-RES-003` ile Inventory Infrastructure, `Sales.Contracts` tarafından yayımlanan confirmed-order snapshot'ını Inventory-owned demand evidence'a çeviren adaptörü aldı. Adaptör producer sonucundaki tenant/company/order/version bağlamını request ile exact karşılaştırır; eksik veya sapmış sonuç fail-closed kalır ve line item/base-UOM/`numeric(20,6)` miktar Inventory değer nesnelerinden geçirilir. Exact order-line seçimiyle state kuran candidate builder sonucu tekrar permission, company, actor-bound warehouse ve demand-capacity invariant'larından geçirir; bulunmayan satırı reddeder. Böylece Inventory Sales tablosuna doğrudan bağlanmadan reservation candidate için authoritative demand kanıtı elde edebilir. Available/persistence, depo seçim politikası ve Sales confirm orchestration henüz uygulanmadı. Inventory Infrastructure, Integration ve Architecture Release derlemeleri `0 warning/error`; runtime MP-04 toplu kapısına bırakıldı.
