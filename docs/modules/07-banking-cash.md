...
- `BNK-REC-003`: Approved reconciliation exact proposal/version workflow kanıtı, hazırlayandan farklı onaylayan ve `0,00` tolerans ister; aynı statement line veya payment ikinci approved sonuçta kullanılamaz.
...
`DEC-MP01-022` ilk ürün politikası: Yetkili kullanıcı kesinleştirdiğinde payment/receipt ekonomik olay ve GL sonucu oluşur. Banka sonucu beklenen harekette ana banka hesabı yerine incoming/outgoing transit hesap kullanılır; statement reconciliation yeni payment üretmeden mevcut hareketi doğrular ve transit hesabı kapatır. Eşleşmeyen satır proposal olarak kalır, ilk sürüm toleransı `0,00`'dır. Banka masrafı, faiz, iade ve chargeback ayrı source event'tir; kasa hareketi banka transit hesabı kullanmaz. Hazırlayan final onaylayan olamaz.
...
Multi-currency payment conversion devamı (1 Eylül 2026): `DEC-MP01-004`–`006` uyarınca payment ekonomik olayı artık TRY/USD/EUR/GBP işlem parası ile TRY functional parayı ayrı taşır. Günlük company-effective rate ve AwayFromZero rounding policy kimlik/sürümleri immutable snapshot'tır; functional amount, 12 haneli unrounded sonuç ve explicit residual yeniden hesaplanabilir. `0042`, eski identity-rate satırlarını ileri uyumlu backfill eder ve DB'de exact conversion guard ekler; runtime UPDATE/DELETE yetkisi açmaz. Gerçek PostgreSQL migration/tamper kanıtı MP validating paketinde beklemektedir. Bu temel, reconciliation sırasındaki payment-date transit değeri ile statement-date banka değeri arasındaki realized FX farkını ayrı journal satırı yapmaya hazırlar; FX journal henüz tamamlanmış değildir.
...
Reconciliation approval uygulama sınırı (31 Ağustos 2026): `BNK-REC-003` ve `DEC-MP01-022` uyarınca yeni Treasury Application kapısı exact proposal identity/version için tamamlanmış workflow kanıtı, `treasury.reconciliation.approve` permission'ı, proposal'ın gerçek maker kimliği ve ondan farklı tam bir yönetici kararı ister. İlk policy quorum'u tam `1`, toleransı `0,00`'dır; dahil edilen her statement line ve payment movement kendi kapasitesine exact kapanmalıdır.

`0041_reconciliation_approval`, approved sonucu proposal'dan ayrı append-only saklar. Approved statement ve payment katılımcıları tenant/company kapsamında tekilleştirilir; aynı satır veya payment ikinci approved reconciliation'a giremez. Deferred DB guard proposal toplamını, workflow subject/maker/decision kanıtını ve movement'ın gerçek immutable payment kimliği, banka hesabı, yönü, para birimi ve tutarını commit anında tekrar doğrular. Reconciliation yeni payment üretmez.

Approved reconciliation→GL composition kanıtı (31 Ağustos 2026): Treasury'nin approved header ve kilitli katılımcılardan yüklediği salt-okunur contract, her statement booking date için ayrı journal source üretir; incoming'de banka kontrol borç/incoming transit alacak, outgoing'de outgoing transit borç/banka kontrol alacak yazılır ve transit satırları payment ID ile drill-down bağını korur. Company/treasury account/chart/rule/account mapping'i caller-supplied fakat Accounting authoritative kapılarında yeniden doğrulanır; proposal-level exact yönetici onayı ayrı statement source'larına taşınır. Approval persistence ile bütün journal prepare/post adımları caller'ın aynı PostgreSQL transaction'ındadır. İlk güvenli dilim TRY identity-rate'tir ve her satır authoritative kur/yuvarlama snapshot'ı taşır. Yabancı para payment-date→statement-date functional farkı ayrı FX source event'i tanımlanmadan fail-closed kalır; bu nedenle USD/EUR/GBP transit kapanışı henüz tamamlanmış sayılmaz.

Realized FX reconciliation devamı (1 Eylül 2026): Approved fact artık matched payment'ın payment-date transaction/TRY functional kapasitesini ve immutable rate/rounding kanıtını taşır. Banka satırı statement booking-date company-effective kuruyla, transit satırı payment-date kuruyla değerlendirilir; fark yönüne göre mapped realized gain/loss hesabında ayrı satırdır. FX hesap kodları uygulamaya gömülmez. Her satır authoritative Accounting rate/policy kontrolünden yeniden geçer. Split payment parçalarının functional toplamı original payment functional tutarına exact eşit değilse gizli residual üretilmez; işlem reddedilir. Standalone kur düzeltmesi hâlâ ayrı onaylı source event ister; buradaki satır yalnız approved reconciliation'ın deterministik realized farkıdır.
