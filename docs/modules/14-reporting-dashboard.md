...
İlk production sorgu sözleşmesi `POST /api/v1/reports/party.account.detail/queries` yoludur. Gövde `companyId`, `statementId` ve `agingReportId` taşır; server `party.account.detail` version `1` tanımını ve `reporting.party-account.view` permission'ını uygular. Permission/company scope resource lookup'tan önce denetlenir. Endpoint projection üretmez; aynı generation'da daha önce atomik yayımlanmış statement/aging cross-foot'unu, source lineage satırlarını, aging policy/bucket/item kanıtını explicit response olarak döndürür. Parasal alanlar JavaScript binary floating-point kaybını önlemek için invariant dört ondalıklı string; event/balance türleri explicit versioned kodlardır. Allowed, denied ve not-found kararları aynı PostgreSQL transaction'ında append-only audit'e yazılmadan sonuç dönmez. Export, print, maliyet ve marj bu permission'a dahil değildir.
...
Şirketin authoritative aging policy tanımı projection snapshot'ından ayrıdır. Tek policy kimliği altında append-only ve ardışık sürümler taşır; rapor kesimi `effective_from <= effective_as_of` ve `recorded_at <= recorded_cutoff` koşullarını sağlayan en yüksek sürümü seçer. Header/bucket guard'ları tam integer-day aralığını, ordinal sıralamayı, sabit policy kimliğini ve stream kronolojisini korur. Definition bulunmazsa job publish etmez; farklı şirket RLS ile görünmez. Seçilen tanım generation sırasında yukarıdaki immutable snapshot'a kopyalanır. Kanıt [authoritative aging policy source planında](../project/plans/2026-08-29-authoritative-aging-policy-source.md) kayıtlıdır.
...
Production Party query adapter'ı bu gate ve audit composition'ını `party.account.detail` version `1` tanımıyla birleştirir. API katmanı yalnız Reporting Application sözleşmesini görür; PostgreSQL transaction sahibi adapter ve ortak audit appender Bootstrap'ta bağlanır. Allowed/not-found transaction'ı normal commit eder; permission denial önce hedefsiz audit'i commit edip typed `Denied` sonucuna dönüşür. Gerçek PostgreSQL ve API/mimari kanıtı [production query planında](../project/plans/2026-08-29-party-report-api-query.md) kayıtlıdır.
...
`RPT-OPS-001`: Party report refresh occurrence'ı PostgreSQL-backed durable work item olarak saklanır. İstek report definition version, effective-as-of, recorded cutoff, deterministic projection kimlikleri, timezone, business-calendar ve missed-run policy snapshot'ını taşır; cari adı veya belge içeriği taşımaz. Worker `FOR UPDATE SKIP LOCKED` ve süreli lease ile tek claim yapar, bounded retry uygular, crash sonrası aynı deterministic generation'ı reclaim eder ve her geçişi append-only event olarak kaydeder. Projection publish'i mevcut source→aging→exact GL→atomic sink zincirinden geçer; servis kimliği ve company permission'ı claim öncesi ve publish öncesi yeniden doğrulanır. Tekrarlayan schedule tanımı/API'si bu occurrence consumer sözleşmesinden ayrıdır ve henüz açık iştir. Uygulama kanıtı [durable Worker planında](../project/plans/2026-08-30-party-report-worker-refresh.md) tutulur.

`DEC-MP01-023`: İlk sürümde occurrence producer, `reporting.party-account.refresh` yetkili kullanıcının idempotent “Raporu yenile” komutudur. Komut HTTP request içinde projection hesaplamaz; deterministic payload'ı durable kuyruğa ekler. Recurring schedule authoring sonraki dilimdir; gelecekteki varsayılan kesim `Europe/Nicosia`, missed-run `run-once` ve ilk sürümde resmi-tatil kaydırması yoktur.

Bir ekonomik olay farklı immutable rapor generation'larında yeniden gösterilebilir; event tekilliği tüm rapor geçmişi üzerinde değil aynı statement içinde uygulanır. `0040_party_statement_projection_event_scope`, aynı statement'ta yinelenen event'i reddederken aynı source cut'ın ayrı generation'larda yeniden üretimini mümkün kılar.
...
Exact Party→GL control evidence güncellemesi (29 Ağustos 2026): Source checksum V2, rapora dahil aktif posted journal lineage setini de kapsar. `IPartyControlAccountEvidenceSource` artık source batch'i doğrudan alır; yalnız report slice + control-account ID ile sentetik veya ilgisiz GL bakiyesi sunulamaz. Reporting PostgreSQL adapter'ı Party fact etkilerinden subledger snapshot'ını bağımsız türetir, Accounting-owned exact-lineage delegate'inden GL snapshot'ını aynı repeatable-read/RLS kesitinde alır ve snapshot kimliklerini generation'dan deterministik üretir. Job, context eşleşse bile reconciliation farkı sıfır değilse `PARTY_CONTROL_ACCOUNT_RECONCILIATION_DIFFERENCE` ile publication'ı durdurur.

Gerçek PostgreSQL kanıtında fonksiyonel parası TRY olan posted `75 GBP` due source source→statement/aging→GL transaction-currency görünümü→atomic sink zincirinden geçti, persisted statement-aging ve subledger-GL tekrar yüklenerek sıfır fark doğrulandı ve aynı command `Created=false` replay oldu. Opening dahil `110 debit - 10 credit = 100 GBP` alt defter/GL mutabakatı ayrıca geçti; yanlış transaction currency snapshot'ı fail-closed reddedildi. Opening event'in due-date/open-item semantiği henüz tanımlı olmadığından opening dahil aging cross-foot'u tahminle yeşile çevrilmedi; bu açık ürün kararı MP-03 planında kayıtlıdır.
