...
Canonical journal preparation varsayılan approval subject'ini source type, source event id ve expected source version'dan server-side türetir. Bir approval birden fazla alt journal üreten immutable composite kaynağa (örneğin reconciliation proposal) aitse command yalnız validated `ApprovalSubjectReference` taşıyabilir; approval subject ile journal source farklı kimlik olsa bile tenant/company kapsamı aynı olmak zorundadır. Exact subject/version authoritative completed approval aynı PostgreSQL transaction'ında ve reservation/draft/audit/outbox'tan önce yüklenir; posted writer da aynı explicit subject'i yeniden doğrular. Caller approval snapshot sağlayamaz, tenant/company değiştiremez ve subject/version kanıtını atlayamaz. Bu preparation hâlâ posted GL sonucu değildir.
...
Control-account exact-lineage okuması (29 Ağustos 2026): Accounting-owned PostgreSQL loader, caller'ın verdiği aktif Party journal setini `journal_id` ile sınırlar; her journal'ın exact source type/event/version/purpose ve effective/recorded/posted kesimini yeniden doğrular. Rapor parası journal fonksiyonel parasıysa GL debit/credit kullanılır; farklıysa her line'ın immutable currency snapshot'ındaki exact transaction currency debit/credit seçilir. Snapshot istenen parayı kanıtlamıyorsa, kesitte etkin reversal varsa veya seçili control account'a hiç satır yoksa fail-closed durur. Loader yalnız GL debit/credit/closing, satır sayısı ve canonical source checksum kanıtı döndürür; Party alt defterini kendisi üretmez ve Reporting tablosuna yazmaz. TRY fonksiyonel journal üzerinde `75 GBP` due ile gerçek job publication, ayrıca `25 opening + 75 due + 10 unallocation - 10 allocation = 100 GBP` transaction-currency control-account sıfır fark senaryosu PostgreSQL/RLS paketinde geçti.
...
Approved banka reconciliation realized FX kanıtı (1 Eylül 2026): Statement journal'ı banka kontrol satırını booking-date rate snapshot'ıyla, transit satırlarını ilgili payment-date rate snapshot'larıyla üretir. Functional fark incoming/outgoing yönüne göre sürümlü mapping'deki realized gain veya loss hesabına ayrı satırdır; bu satır TRY identity rate ve rounding snapshot'ı taşır. Payment split'lerinin functional toplamı immutable payment functional kapasitesini exact kapatmıyorsa posting reddedilir. Hesap kodu, kur veya residual hard-code edilmez.
